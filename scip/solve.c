...
{  /*lint --e{715}*/
   ...
   //printf("calling primal heuristics... \n");
   ...
   *foundsol = FALSE;
   ...
   else
   {
      ...
      nodenumber = SCIPnodeGetNumber(SCIPtreeGetFocusNode(tree));
   ...
   for( h = 0; h < set->nheurs; ++h )
   {
      ...
      /*
      {
         ...
         if( SCIPheurGetFreq(set->heurs[h]) != -1 )
            printf("checking heuristic <%s> \n", SCIPheurGetName(set->heurs[h]));

         if( SCIPheurShouldBeExecuted(set->heurs[h], depth, lpstateforkdepth, heurtiming, &delayed) )
         {
            printf(" -> executing heuristic <%s> with priority %d\n",
               SCIPheurGetName(set->heurs[h]), SCIPheurGetPriority(set->heurs[h]));
            if( delayed )
               printf(" .... delayed \n");
         }
         else if( SCIPheurGetFreq(set->heurs[h]) != -1 )
         {
            printf(" -----------> heuristic <%s> is not going to be executed \n", SCIPheurGetName(set->heurs[h]));
         }
      }
      */
      SCIP_CALL( SCIPheurExec(set->heurs[h], set, primal, nodenumber, depth, lpstateforkdepth, heurtiming, nodeinfeasible,
      ...
      if( heurtiming == SCIP_HEURTIMING_BEFOREPRESOL )
         printf("we are in BEFOREPRESOL \n");
      if( heurtiming == SCIP_HEURTIMING_DURINGPRESOLLOOP )
         printf("we are in DURINGPRESOLLOOP \n");
      if( heurtiming == SCIP_HEURTIMING_AFTERPROPLOOP )
         printf("we are in AFTERPROPLOOP \n");
      if( heurtiming == SCIP_HEURTIMING_BEFORENODE )
         printf("we are in BEFORENODE \n");
      if( heurtiming == SCIP_HEURTIMING_DURINGLPLOOP )
         printf("we are in DURINGLPLOOP \n");
      if( heurtiming == SCIP_HEURTIMING_DURINGPRICINGLOOP )
         printf("we are in DURINGPRICINGLOOP \n");
      if( heurtiming == SCIP_HEURTIMING_AFTERLPLOOP )
         printf("we are in AFTERLPLOOP \n");
      if( heurtiming == SCIP_HEURTIMING_AFTERNODE )
         printf("we are in AFTERNODE \n");
      else
         printf("no idea whats going on. \n");
      //printf("***** heurtiming == SCIP_HEURTIMING_DURINGPRICINGLOOP ? %d \n", heurtiming == SCIP_HEURTIMING_DURINGPRICINGLOOP);
      */
