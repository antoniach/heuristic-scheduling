   ...
   )
{
   ...
   /* introduce new parameters to influence how long the dive runs for */
   (void) SCIPsnprintf(paramname, SCIP_MAXSTRLEN, "heuristics/%s/nbacktracks", diveset->name);
   SCIP_CALL( SCIPsetAddIntParam(set, messagehdlr, blkmem,
         paramname,
         "if backtracking is used, how often? (1: scip default)",
         &diveset->nbacktracks, FALSE, 1, 0, INT_MAX, NULL, NULL) );

   /*
   (void) SCIPsnprintf(paramname, SCIP_MAXSTRLEN, "heuristics/%s/maxlpiter", diveset->name);
   SCIP_CALL( SCIPsetAddIntParam(set, messagehdlr, blkmem,
         paramname,
         "for how many LP iterations should the diving heuristic run? (-1: scip default)",
         &diveset->maxlpiter, FALSE, -1, 0, INT_MAX, NULL, NULL) );
   */

   (void) SCIPsnprintf(paramname, SCIP_MAXSTRLEN, "heuristics/%s/maxdivedepth", diveset->name);
   SCIP_CALL( SCIPsetAddIntParam(set, messagehdlr, blkmem,
         paramname,
         "how deep should we at most dive? (-1: scip default)",
         &diveset->maxdivedepth, FALSE, -1, -1, INT_MAX, NULL, NULL) );

   (void) SCIPsnprintf(paramname, SCIP_MAXSTRLEN, "heuristics/%s/timelimit", diveset->name);
   SCIP_CALL( SCIPsetAddRealParam(set, messagehdlr, blkmem, paramname,
         "limit the time spent on a dive",
         &diveset->timelimit, FALSE, SCIP_REAL_MAX, 0.0, SCIP_REAL_MAX, NULL, NULL) );

   return SCIP_OKAY;
}

char* SCIPdivingdataGetHeurName(
   SCIP_DIVINGDATA*      divingdata          /**< data structure for data collection */
   )
{
   assert(divingdata != NULL);

   return divingdata->heurname;
}

int SCIPdivingdataGetNSols(
   SCIP_DIVINGDATA*      divingdata          /**< data structure for data collection */
   )
{
   assert(divingdata != NULL);

   return divingdata->nsolsfound;
}

SCIP_Longint SCIPdivingdataGetNodeNumber(
   SCIP_DIVINGDATA*      divingdata          /**< data structure for data collection */
   )
{
   assert(divingdata != NULL);

   return divingdata->nodenumber;
}

int SCIPdivingdataGetNodeDepth(
   SCIP_DIVINGDATA*      divingdata          /**< data structure for data collection */
   )
{
   assert(divingdata != NULL);

   return divingdata->nodedepth;
}

int SCIPdivingdataGetNBestsols(
   SCIP_DIVINGDATA*      divingdata          /**< data structure for data collection */
   )
{
   assert(divingdata != NULL);

   return divingdata->nbestsolsfound;
}

int SCIPdivingdataGetDivingDepth(
   SCIP_DIVINGDATA*      divingdata          /**< data structure for data collection */
   )
{
   assert(divingdata != NULL);

   return divingdata->divingdepth;
}

SCIP_Real SCIPdivingdataGetBestPrimVal(
   SCIP_DIVINGDATA*      divingdata          /**< data structure for data collection */
   )
{
   assert(divingdata != NULL);

   return divingdata->bestprimval;
}

SCIP_Real SCIPdivingdataGetLPresolvedomchgquot(
   SCIP_DIVINGDATA*      divingdata          /**< data structure for data collection */
   )
{
   assert(divingdata != NULL);

   return divingdata->lpresolvedomchgquot;
}

SCIP_Real SCIPdivingdataGetTime(
   SCIP_DIVINGDATA*      divingdata          /**< data structure for data collection */
   )
{
   assert(divingdata != NULL);

   return divingdata->time;
}

SCIP_Real SCIPdivingdataGetSolvingTime(
   SCIP_DIVINGDATA*      divingdata          /**< data structure for data collection */
   )
{
   assert(divingdata != NULL);

   return divingdata->solvingtime;
}

void SCIPdivingdataIncreaseNSols(
   SCIP_DIVINGDATA*      divingdata          /**< data structure for data collection */
   )
{
   assert(divingdata != NULL);

   divingdata->nsolsfound++;
}

void SCIPdivingdataIncreaseNBestsols(
   SCIP_DIVINGDATA*      divingdata          /**< data structure for data collection */
   )
{
   assert(divingdata != NULL);

   divingdata->nbestsolsfound++;
}

void SCIPdivingdataSetDivingDepth(
   SCIP_DIVINGDATA*      divingdata,         /**< data structure for data collection */
   int                   value               /**< value the depth should be set to */
   )
{
   assert(divingdata != NULL);

   divingdata->divingdepth = value;
}

int SCIPdivingdataGetNNodes(
   SCIP_DIVINGDATA*      divingdata          /**< data structure for data collection */
   )
{
   assert(divingdata != NULL);

   return divingdata->nnodes;
}

void SCIPdivingdataSetNNodes(
   SCIP_DIVINGDATA*      divingdata,         /**< data structure for data collection */
   int                   value               /**< value nnods should be set to */
   )
{
   assert(divingdata != NULL);

   divingdata->nnodes = value;
}

void SCIPdivingdataSetBestPrimVal(
   SCIP_DIVINGDATA*      divingdata,         /**< data structure for data collection */
   SCIP_Real             value               /**< value to set to */
   )
{
   assert(divingdata != NULL);

   divingdata->bestprimval = value;
}
...
/** limit time spent on a dive */
SCIP_Real SCIPdivesetGetTimeLimit(
   SCIP_DIVESET*         diveset             /**< diving settings */
   )
{
   return diveset->timelimit;
}
...
/** get the maximal level of backtracking */
int SCIPdivesetGetBacktrackLimit(
   SCIP_DIVESET*         diveset             /**< diving settings */
   )
{
   return diveset->nbacktracks;
}

/** how deep whould we at most dive? */
int SCIPdivesetGetMaxdivedepth(
   SCIP_DIVESET*         diveset             /**< diving settings */
   )
{
   return diveset->maxdivedepth;
}
   ...
   )
{
   ...
   (void) SCIPsnprintf(paramname, SCIP_MAXSTRLEN, "heuristics/%s/shadowmode", name);
   SCIP_CALL( SCIPsetAddBoolParam(set, messagehdlr, blkmem, paramname,
                  "should shadow mode be used? (i.e. solutions found by heuristics are not reported back)",
                  &(*heur)->shadowmode, FALSE, FALSE, NULL, NULL) );
   ...
   )
{
   ...
   else
   {
      ...
      //if( heur->freq != -1 )
      //{
      //   printf("here3, execute = %d \n", execute);
      //   printf("freq = %d, ofs = %d, depth = %d \n",heur->freq, heur->freqofs, depth);
      //}
   }
   ...
   if(heur->freq != -1)
   {
      printf("timingmask = %d and heurtiming = %d \n", heur->timingmask, heurtiming);
      printf("execute = %d before returning \n", execute);
   }
   */
...
void SCIPwriteDivingData(
   SCIP_DIVINGDATA*      divingdata,         /**< diving data for data collection */
   char*                 filename
   )
{
   FILE* file;
   ...
   //printf("%d \n", divingdata->time);
   fprintf(file, "%-17.17s  |  %10lld  |  %d  |  %d  |  %d  | %f \n", divingdata->heurname, divingdata->nodenumber, divingdata->nsolsfound,
         divingdata->nbestsolsfound, divingdata->divingdepth, divingdata->time);
...
SCIP_RETCODE SCIPheurExec(
   ...
   SCIP_Longint          nodenumber,         /**< node number of current node */
   int                   depth,              /**< depth of current node */
   int                   lpstateforkdepth,   /**< depth of the last node with solved LP */
   SCIP_HEURTIMING       heurtiming,         /**< current point in the node solving process */
   ...
   )
{
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   //if( execute )
   //{
   //   printf(" -> executing heuristic <%s> with priority %d and freq %d\n", SCIPheurGetName(heur), SCIPheurGetPriority(heur), SCIPheurGetFreq(heur));
   //   if( delayed )
   //      printf(" .... delayed \n");
   //}
//   else if( SCIPheurGetFreq(heur) != -1 )
//      printf(" -----------> heuristic <%s> is not going to be executed \n", SCIPheurGetName(heur));

   //if( SCIPheurGetFreq(heur) != -1 )
   ...
   if( execute )
   {
      ...
      if( SCIPheurUsesShadowMode(heur) )
      {
         //SCIP_Real lpresolvedomchgquot[8] = {0.6, 0.3, 0.1, 0.07, 0.04, 0.01, 0.005, 0.15};
         SCIP_Real lpresolvedomchgquot[1] = {0.15};
         int size = 1;
         int i;

         for( i = 0; i < size; ++i )
         {
            SCIP_Real oldruntime;

            /* create DivingData data structure */
            //printf("initializing diving data \n");
            divingdata = (SCIP_DIVINGDATA*) malloc(sizeof(SCIP_DIVINGDATA));
            //printf("done initializing diving data \n");
            divingdata->nsolsfound = 0;
            divingdata->nbestsolsfound = 0;
            divingdata->lpresolvedomchgquot = lpresolvedomchgquot[i];
            divingdata->bestprimval = SCIPinfinity(set->scip);
            divingdata->nnodes = 0;

            /* start timing */
            oldruntime = SCIPheurGetTime(heur);
            SCIPclockStart(heur->heurclock, set);

            /* call external method */
            SCIP_CALL( heur->heurexec(set->scip, heur, heurtiming, nodeinfeasible, divingdata, result) );
            ...
            {
               //printf("heuristic ran\n");
               //printf("%d \n", divingdata->lpresolvefreq);
               divingdata->heurname = heur->name;
               divingdata->nodenumber = nodenumber;
               divingdata->time = SCIPheurGetTime(heur) - oldruntime;
               divingdata->nodedepth = depth;

               //printf("time = %f, depth = %d, lpresolvefreq = %d \n", divingdata->time, divingdata->divingdepth, divingdata->lpresolvefreq);
               set->nheurcalls++;

               if( set->nheurcalls > set->heurcallssize )
               {
                  int newsize;

                  newsize = SCIPcalcMemGrowSize(set->scip, set->nheurcalls);
                  SCIP_CALL( SCIPreallocBlockMemoryArray(set->scip, &set->heurcalls, set->heurcallssize, newsize) );
                  set->heurcallssize = newsize;
               }
               assert(set->nheurcalls <= set->heurcallssize);

               set->heurcalls[set->nheurcalls - 1] = divingdata;
            }
            else
            {
               free(divingdata);
...
/** is primal heuristic using shadow mode? */
SCIP_Bool SCIPheurUsesShadowMode(
   SCIP_HEUR*            heur                /**< primal heuristic */
   )
{
   assert(heur != NULL);

   return heur->shadowmode;
}
