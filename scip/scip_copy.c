 ...
 * are represented as NULL entry in the \p subvars array.
 */
SCIP_RETCODE SCIPtranslateSubSolsShadowMode(
   SCIP*                 scip,               /**< the SCIP data structure */
   SCIP*                 subscip,            /**< SCIP data structure of the subproblem */
   SCIP_HEUR*            heur,               /**< heuristic that found the solution */
   SCIP_VAR**            subvars,            /**< the variables from the subproblem in the same order as the main \p scip */
   SCIP_SOL**            sol,                /**< pointer to store solution that would have been added or NULL */
   ...
   for( i = 0; i < nsubsols; ++i )
   {
      ...
      if( *success )
      {
         ...
         *feasible = TRUE;
         ...
         if( SCIPgetSolOrigObj(scip, newsol) < bestval )
         {
            bestval = SCIPgetSolOrigObj(scip, newsol);
            *sol = newsol;
         }

         SCIP_CALL( SCIPaddSolFreeShadowMode(scip, &newsol, success) );
         if( *success )
         {
            SCIPdebugMsg(scip, "-> accepted solution of value %g\n", SCIPgetSolOrigObj(subscip, subsols[i]));
            *sol = newsol;
