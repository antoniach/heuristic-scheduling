...
SCIP_RETCODE SCIPperformGenericDivingAlgorithm1(
   SCIP*                 scip,               /**< SCIP data structure */
...
SCIP_RETCODE SCIPperformGenericDivingAlgorithm(
   ...
   SCIP_SOL*             worksol,            /**< non-NULL working solution */
   SCIP_HEUR*            heur,               /**< the calling primal heuristic */
   SCIP_DIVINGDATA*      divingdata,         /**< data structure for data collection or NULL */
   SCIP_RESULT*          result,             /**< SCIP result pointer */
   SCIP_Bool             nodeinfeasible,     /**< is the current node known to be infeasible? */
   SCIP_Longint          iterlim,            /**< nonnegative iteration limit for the LP solves, or -1 for dynamic setting */
   SCIP_DIVECONTEXT      divecontext         /**< context for diving statistics */
   )
{
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   maxdivedepth = SCIPdivesetGetMaxdivedepth(diveset);

   /* set lpresolvedomchgquot */
   if( divingdata == NULL )
      lpresolvedomchgquot = SCIPdivesetGetLPResolveDomChgQuot(diveset);
   else
      lpresolvedomchgquot = SCIPdivingdataGetLPresolvedomchgquot(divingdata);
   ...
   *result = SCIP_DIDNOTFIND;
         ...
         && !SCIPisStopped(scip) )
   {
      ...
      //printf("oldtime = %f \n", oldtime);
      //printf("currenttime = %f \n", SCIPheurGetTime(heur));
      //printf("time limit = %f \n", SCIPdivesetGetTimeLimit(diveset));
      //printf("difference = %f \n", SCIPheurGetTime(heur) - oldtime);
      if( (SCIPheurGetTime(heur) - oldtime) > SCIPdivesetGetTimeLimit(diveset) )
      {
         //printf("here \n");
         goto TERMINATE;
      }
      ...
      if( allroundable )
      {
         ...
         if( success )
         {
            ...
            if( SCIPheurUsesShadowMode(heur) )
            {
               SCIP_Bool feasible;
               feasible = FALSE;
               SCIP_CALL( SCIPtrySolShadowMode(scip, worksol, FALSE, FALSE, FALSE, FALSE, changed, &feasible, &success) );

               if( feasible )
               {
                  SCIPdivingdataIncreaseNSols(divingdata);
                  SCIPdivingdataSetBestPrimVal(divingdata, SCIPgetSolOrigObj(scip, worksol));
               }
               if( success )
               {
                  /* incumbent was found: update counter and track objective value */
                  SCIPdivingdataIncreaseNBestsols(divingdata);

                  //if( SCIPisInfinity(scip, SCIPdivingdataGetBestPrimVal(divingdata))
                  //   || (SCIPgetObjsense(scip) == SCIP_OBJSENSE_MAXIMIZE && SCIPdivingdataGetBestPrimVal(divingdata) < SCIPgetSolOrigObj(scip, worksol))
                  //   || (SCIPgetObjsense(scip) == SCIP_OBJSENSE_MINIMIZE && SCIPdivingdataGetBestPrimVal(divingdata) > SCIPgetSolOrigObj(scip, worksol)) )
                  //{
                  //   SCIPdivingdataSetBestPrimVal(divingdata, SCIPgetSolOrigObj(scip, worksol));
                  //}
      ...
      do
      {
         ...
         do
         {
            ...
            if( ! cutoff
                  ...
                  || (domreds + localdomreds > lpresolvedomchgquot * SCIPgetNVars(scip))
                  || (onlylpbranchcands && nviollpcands > (int)(lpresolvedomchgquot * nlpcands))) )
            {
            ...
            //printf("cutoff = %d, and backtracked = %d \n", cutoff, backtracked);
            /* perform backtracking if a cutoff was detected */
            if( cutoff && nbacktracks <= SCIPdivesetGetBacktrackLimit(diveset) && SCIPdivesetUseBacktrack(diveset) )
            {
               ...
               if( nbacktracks > SCIPdivesetGetBacktrackLimit(diveset) )
                  backtracked = TRUE;
               else
                  backtracked = FALSE;
            }
   ...
   //printf("*****************************************************lperror = %d, cutoff = %d, too deep = %d, too many iterations = %d",lperror,cutoff,SCIPgetProbingDepth(scip) >= maxdivedepth,SCIPdivesetGetNLPIterations(diveset, divecontext) >= maxnlpiterations);
   ...
   if( !enfosuccess && !lperror && !cutoff && SCIPgetLPSolstat(scip) == SCIP_LPSOLSTAT_OPTIMAL )
   {
      ...
      /* try to add solution to SCIP */
      //printf("shadow mode = %d \n", SCIPdivesetUseShadowMode(diveset));
      ...
      if( SCIPheurUsesShadowMode(heur) )
      {
         ...
         //printf("here2 \n");
         feasible = FALSE;
         SCIP_CALL( SCIPtrySolShadowMode(scip, worksol, FALSE, FALSE, FALSE, FALSE, FALSE, &feasible, &success) );

         if( feasible )
         {
            /* solution was found: update counter */
            SCIPdivingdataIncreaseNSols(divingdata);
            SCIPdivingdataSetBestPrimVal(divingdata, SCIPgetSolOrigObj(scip, worksol));
         }
         if( success )
   ...
   if(divingdata != NULL )
      SCIPdivingdataSetDivingDepth(divingdata, SCIPgetDepth(scip));
