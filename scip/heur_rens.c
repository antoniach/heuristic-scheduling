   ...
   )
{
   ...
   if( SCIPheurUsesShadowMode(heur) )
      SCIP_CALL( SCIPsetRealParam(subscip, "limits/time", 15) );
   ...
   if( allfixingrate >= minfixingrate / 2.0 )
   {
      ...
      for( i = 0; i < nsubsols && (!success || heurdata->addallsols); ++i )
      {
         ...
         if( SCIPheurUsesShadowMode(heur) )
         {
            ...
            SCIPdivingdataSetNNodes(divingdata, SCIPgetNNodes(subscip));

            SCIP_CALL( SCIPtrySolFreeShadowMode(scip, &newsol, FALSE, FALSE, TRUE, TRUE, TRUE, &feasible, &success) );

            if( feasible )
            {
               /* solution was found: update counter */
               SCIPdivingdataIncreaseNSols(divingdata);
               SCIPdivingdataSetBestPrimVal(divingdata, SCIPgetSolOrigObj(scip, newsol));
            }
            if( success )
            {
   ...
   )
{
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   retcode = setupAndSolveSubscip(scip, subscip, result, heur, fixedvars, fixedvals, nfixedvars, intfixingrate, minfixingrate, minimprove, maxnodes, nstallnodes, startsol, binarybounds, uselprows, divingdata);
...
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   SCIP_CALL( SCIPapplyRens(scip, heur, result, heurdata->minfixingrate, heurdata->minimprove,
         heurdata->maxnodes, nstallnodes, heurdata->startsol, heurdata->binarybounds, heurdata->uselprows, divingdata) );
